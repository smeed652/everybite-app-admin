...
const metaEnv: any | undefined =
  typeof import.meta === "object" && (import.meta as any).env
...
const userPoolId = (metaEnv?.VITE_COGNITO_USER_POOL_ID ??
  nodeEnv?.VITE_COGNITO_USER_POOL_ID) as string;
const userPoolWebClientId = (metaEnv?.VITE_COGNITO_APP_CLIENT_ID ??
  nodeEnv?.VITE_COGNITO_APP_CLIENT_ID) as string;

if (!region || !userPoolId || !userPoolWebClientId) {
  logger.warn(
    "[aws-auth] Missing Cognito env vars – authentication will not work until they are set."
  );
}
